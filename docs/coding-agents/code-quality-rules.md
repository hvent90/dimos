...
* Do no work at import time: no subprocesses, viewers, model parsing, or network. In particular don't call `get_data(...)` (it blocks import until the download finishes) -- use `LfsPath` for large non-description LFS assets, `robot_description_path(...)` for built-in robot descriptions, or build the config in `start`/`build`. Any process you start must be managed (shut down when not needed).
