...
/// Stdin JSON wrapper from the Python native-module launcher (`{"config": ...}`).
#[derive(Deserialize)]
struct Envelope {
    config: Config,
}
...
struct NativeTopicConfig {
    ...
    colors: Option<[u8; 3]>,
    mode: PointCloudMode,
    fill_mode: PointCloudFillMode,
    bottom_cutoff: Option<f32>,
}

const DEFAULT_TOPIC_CONFIG: NativeTopicConfig = NativeTopicConfig {
    voxel_size: 0.05,
    colors: None,
    mode: PointCloudMode::Spheres,
    fill_mode: PointCloudFillMode::Solid,
    bottom_cutoff: None,
};

type LogMessage = fn(&Sink, &str, Vec<u8>, NativeTopicConfig) -> Result<String>;

const NATIVE_TYPES: [(&str, LogMessage); 2] = [
    ("sensor_msgs.Image", Sink::log_image),
    ("sensor_msgs.PointCloud2", Sink::log_pointcloud),
];

/// Latest-wins packet buffer between the LCM/Zenoh listener and the log loop.
struct Latest {
...
/// Decodes native heavy payloads and logs them to the Rerun recording stream.
struct Sink {
...
impl Sink {
    fn new(config: &Config) -> Result<Self> {
        let builder =
            rerun::RecordingStreamBuilder::new("dimos").recording_id(config.recording_id.clone());
        let recording = match &config.save_path {
            Some(save_path) => builder.save(save_path)?,
            None => builder.connect_grpc_opts(config.connect_url.clone())?,
        };
    ...
    fn log(&mut self, channel: &str, payload: Vec<u8>) -> Result<()> {
        ...
        let topic_config = self
            ...
            .get(&entity)
            .copied()
            .unwrap_or(DEFAULT_TOPIC_CONFIG);
        ...
        let frame_id = log(self, &entity, payload, topic_config)?;
    ...
    fn log_pointcloud(
        ...
        for point in cloud.data.chunks_exact(step).take(count) {
            ...
            if let Some(bottom_cutoff) = config.bottom_cutoff {
                if position[2] < bottom_cutoff {
                    continue;
                }
            }
        ...
        let class_ids: Vec<u16> = points
            ...
            .collect();
        let radius = config.voxel_size / 2.0;
        match config.mode {
            PointCloudMode::Boxes => {
                let fill_mode = match config.fill_mode {
                    PointCloudFillMode::Solid => FillMode::Solid,
                    PointCloudFillMode::Majorwireframe => FillMode::MajorWireframe,
                    PointCloudFillMode::Densewireframe => FillMode::DenseWireframe,
                };
                let boxes =
                    rerun::Boxes3D::from_centers_and_half_sizes(points, [[radius, radius, radius]])
                        .with_fill_mode(fill_mode);
                match config.colors {
                    Some(color) => self.recording.log(entity, &boxes.with_colors([color]))?,
                    None => self
                        .recording
                        .log(entity, &boxes.with_class_ids(class_ids))?,
                }
            }
            // "points" and "spheres" both match Python PointCloud2.to_rerun: Points3D
            // with radii.
            PointCloudMode::Points | PointCloudMode::Spheres => {
                let points = rerun::Points3D::new(points).with_radii([radius]);
                match config.colors {
                    Some(color) => self.recording.log(entity, &points.with_colors([color]))?,
                    None => self
                        .recording
                        .log(entity, &points.with_class_ids(class_ids))?,
                }
            }
        }
...
fn classify(channel: &str) -> Option<(LogMessage, &str)> {
    let (topic, type_name) = if let Some(parts) = channel.rsplit_once('#') {
        parts
    } else {
        channel.strip_prefix("dimos-heavy")?.rsplit_once('/')?
    };
    NATIVE_TYPES
        .iter()
...
async fn listen_zenoh(latest: Arc<Latest>, config: &Config) -> Result<()> {
    ...
    let _subscriber = session
        .declare_subscriber("dimos-heavy/**")
...
async fn main() -> Result<()> {
    ...
    let heavy_types: HashSet<&str> = envelope
        .config
        .heavy_types
        .iter()
        .map(String::as_str)
        .collect();
    let native_types: HashSet<&str> = NATIVE_TYPES.iter().map(|(name, _)| *name).collect();
    if heavy_types != native_types {
        bail!("heavy type mismatch: Python {heavy_types:?}, Rust {native_types:?}");
    }
