...
For return visits to a known space, use [premap relocalization](/docs-old/capabilities/navigation/relocalization.md) instead of relying on live mapping alone.
...
Live column-carving has no loop closure. We trust Go2 odometry, which is stable but drifts over distance. You can reliably map and navigate large spaces around 500 m² in our tests, but not kilometer-scale outdoor routes. For return visits with loop-closed maps, use [premap relocalization](/docs-old/capabilities/navigation/relocalization.md) and build the premap offline with `dimos map global --export`.
