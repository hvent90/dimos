...
1. **CLI:** `dimos --transport=zenoh ...` or `dimos --transport=lcm ...` (see [CLI](/docs-old/usage/cli.md) for precedence with `.env` and blueprints).
...
See [Blueprints](/docs-old/usage/blueprints.md) for the blueprint API.
...
See [Modules](/docs-old/usage/modules.md) for more on module architecture.
...
Many of our message types provide `lcm_encode` / `lcm_decode` for compact, language-agnostic binary encoding (often faster than pickle). For details, see [LCM](/docs-old/usage/lcm.md).
...
Topic/message types are flexible: bytes, JSON, or our ROS-compatible [LCM](/docs-old/usage/lcm.md) types. We also have pickle-based transports for arbitrary Python objects.
...
`LCMEncoderMixin` is especially useful: you can use LCM message definitions with *any* transport (not just UDP multicast). See [LCM](/docs-old/usage/lcm.md) for details.
