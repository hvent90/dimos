...
renders the regions, and optionally persists via RoomStore::
    ...
    uv run python dimos/mapping/occupancy/rooms/tool_replay.py \
...
from dimos.mapping.occupancy.rooms.segmentation import render_regions, segment_rooms
...
def main() -> None:
    ...
    if args.store:
        with RoomStore(args.store) as store:
            store.save(segmentation, source=f"rooms.tool_replay:{args.db}")
