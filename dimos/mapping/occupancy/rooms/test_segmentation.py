...
def test_unexplored_space_is_never_segmented() -> None:
    # A door onto a room the robot has not entered yet. The EDT is over free
    # space, so unknown acts as a wall: nothing seeds or floods into it.
    cells = _walled(84, 166)
    cells[:, 82:84] = 100  # dividing wall
    cells[34:50, 82:84] = 0  # door gap
    cells[2:82, 84:164] = -1  # everything past the door is unobserved
    seg = segment_rooms(_grid(cells))
    assert len(seg.regions) == 1
    assert (seg.labels > 0)[cells == -1].sum() == 0
