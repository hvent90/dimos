...
def test_polygon_to_flat_round_trips() -> None:
    flat = polygon_to_flat(UNIT_SQUARE)
    assert flat == [0.0, 0.0, 1.0, 0.0, 1.0, 1.0, 0.0, 1.0]
    np.testing.assert_array_equal(polygon_from_flat(flat), UNIT_SQUARE)


def test_polygon_to_flat_rounds_to_millimeters() -> None:
    noisy = UNIT_SQUARE + 1e-9
    assert polygon_to_flat(noisy) == polygon_to_flat(UNIT_SQUARE)
    assert polygon_to_flat(np.array([[0.12345, 0.0], [1.0, 0.0], [1.0, 1.0]]))[0] == 0.123
