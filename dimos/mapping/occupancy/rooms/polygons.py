...
def polygon_to_flat(polygon: NDArray[np.float64], ndigits: int = 3) -> list[float]:
    """Flatten an (M, 2) polygon to a [x1, y1, x2, y2, ...] list for storage.

    Rounded to the millimeter by default: these are lidar-derived outlines in
    meters, so the trailing float noise is neither meaningful nor worth the
    bytes it costs in every stored copy.
    """
    return [round(float(v), ndigits) for v in np.asarray(polygon, dtype=np.float64).ravel()]


def cells_to_world(
    cells_xy: ArrayLike, resolution: float, origin_xy: tuple[float, float]
) -> NDArray[np.float64]:
    """World coordinates of cell *centers*, from (..., 2) (x, y) cell indices.

    Fractional indices are allowed, so the mean of a cluster of cells converts
    the same way a single cell does.

    Args:
        cells_xy: Cell indices as (x, y), any shape ending in 2.
        resolution: Cell size in meters.
        origin_xy: World coordinates of the map's (0, 0) cell corner.
    """
    world: NDArray[np.float64] = (np.asarray(cells_xy, dtype=np.float64) + 0.5) * resolution
    return world + np.asarray(origin_xy, dtype=np.float64)
...
def mask_to_polygon(
    ...
    return cells_to_world(cells, resolution, origin_xy)
