...
class ModuleCoordinator(Resource):
    """Deploys modules and wires them together.

    All module state is keyed by the *instance name* (`BlueprintAtom.name`,
    which defaults to the lowercased class name), so the same module class can
    be deployed multiple times under different names. Public methods that take
    a module class resolve it to the single deployed instance of that class
    and raise if there is more than one.
    """
    ...
    def __init__(
        ...
        self._module_transports: dict[str, dict[str, PubSubTransport[Any]]] = {}
        self._started = False
    ...
    def _load_blueprint(
        ...
        _connect_module_refs(blueprint, self, existing_modules=existing_classes)
