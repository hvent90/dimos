...
run can be appended and compared on one timeline, and replaces only its own
streams when appending (``force``). Poses come straight from the odometry stream
(``@pose_setter_for``): each lidar frame is stamped with the latest odometry pose
so ``fastlio_lidar`` carries the trajectory and ``dimos map global`` can register
it.
"""
