...
"""Wavefront local planner — a wavefront *navigation function* over a
clearance-shaped cost field.
...
the wavefront but uses it the robust way: as a **cost layer** searched
...
* **Clearance + obstacle avoidance** — the wavefront is folded into the
...
planner core. ``WavefrontLocalPlanner`` is the DimOS ``Module`` wrapper.
"""
...
class WavefrontParams:
    ...
    influence_radius: float = 0.8  # clearance cost ramps up within this *beyond* the body
...
def _obstacle_distance(
    cost: np.ndarray, resolution: float, params: WavefrontParams
...
def _clearance_penalty(
    ...
    """Soft clearance cost ramping 0 -> 1 as clearance beyond the body shrinks to 0.
...
def clearance_cost(cost: np.ndarray, resolution: float, params: WavefrontParams) -> np.ndarray:
    """The wavefront as a bounded cost layer (the obstacle-proximity term).
...
def _assign_headings(
    points: list[np.ndarray], goal: np.ndarray, params: WavefrontParams
...
def plan_path(
    ...
    """Plan a local path of oriented poses with a clearance-cost navigation function.
    ...
    # Cost layer = base travel + clearance + global-path adherence.
    entry_cost = (
        1.0
...
class WavefrontLocalPlannerConfig(ModuleConfig):
    """Config for :class:`WavefrontLocalPlanner` (mirrors the params)."""
    ...
    # WavefrontParams keep 0 = off so offline tools stay whole-map/unbounded):
    ...
    def to_params(self, speed: float = 0.0, horizon_boost: float = 0.0) -> WavefrontParams:
...
class WavefrontLocalPlanner(Module):
    ...
    over a clearance-shaped cost field — loop- and local-minimum-free by construction).
    """

    config: WavefrontLocalPlannerConfig
    ...
    async def main(self) -> AsyncIterator[None]:
        ...
        self._emit_thread = threading.Thread(
            target=self._emit_loop, daemon=True, name="WavefrontEmitter"
        )
