...
"""Evaluate a loop-closure / PGO module against a recording.
...
Usage:
    uv run python dimos/navigation/nav_stack/modules/pgo/eval.py \\
        ...
        --module-path dimos/navigation/nav_stack/modules/pgo/pgo.py \\
        --module-name PGO \\
...
from dimos.navigation.nav_stack.modules.pgo.eval_utils.apriltag_agreement import (
    VISIT_GAP_S,
    AgreementReport,
...
from dimos.navigation.nav_stack.modules.pgo.eval_utils.recording_db import (
    MAX_REPLAY_ODOM,
    MAX_REPLAY_SCANS,
...
from dimos.navigation.nav_stack.modules.pgo.eval_utils.trajectory_metrics import (
    GraphPose,
    PoseLookup7,
    drifted_lookup,
    graph_lookup,
...
# for walks where the same spot is re-seen tens of seconds later. Applied on top
# of whatever defaults the module-under-test ships; filter_config_for_module
# drops any key the module doesn't declare.
DEFAULT_PGO_CONFIG: dict[str, Any] = {
    ...
    # (Dropped by filter_config_for_module for modules that lack the knob.)
    ...
    async def _replay(self, messages: list[tuple[float, str, Any]]) -> None:
        ...
        for timestamp, kind, payload in messages:
            ...
            self.registered_scan.publish(
                PointCloud2.from_numpy(points, frame_id="map", timestamp=timestamp)
            )
            scans_sent += 1
...
def run_module_graph(
    ...
    *,
    ...
    output_path = Path(tempfile.gettempdir()) / f"pgo_lc_eval_{db_path.parent.name}.json"
    output_path.unlink(missing_ok=True)
    done_path = Path(tempfile.gettempdir()) / f"pgo_lc_eval_done_{db_path.parent.name}.json"
...
def write_trajectory_rrd(rrd_path: Path, raw_positions: np.ndarray, graph: list[GraphPose]) -> None:
    ...
    rr.init("nav_stack_pgo_eval", spawn=False)
...
if __name__ == "__main__":
    ...
    importlib.import_module("dimos.navigation.nav_stack.modules.pgo.eval").main()
