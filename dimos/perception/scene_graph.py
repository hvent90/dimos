...
from dimos.mapping.occupancy.rooms.geometry import RegionGeometry
...
class SceneNode:
    ...
    def polygon(self) -> NDArray[np.float64]:
        ...
        return polygon_from_flat(self.extent)
...
def _match_regions(old_regions: list[SceneNode], rooms: list[StoredRoom]) -> dict[int, str]:
    ...
    The same-place test itself is geometry, and lives in
    :func:`dimos.mapping.occupancy.rooms.matching.match_regions`; this adapts
    graph nodes to outlines and maps the answer back onto ids. Nodes with no
    measured outline can't be matched and don't take part.
    """
    measurable = [n for n in old_regions if n.extent is not None]
    matched = match_regions(
        [RegionOutline(n.layer, n.polygon(), _region_centroid(n)) for n in measurable],
        [RegionOutline(r.kind, r.polygon, r.centroid_xy) for r in rooms],
    )
    return {rooms[derived].id: measurable[previous].id for derived, previous in matched.items()}
...
class SceneGraph:
            ...
            if existing_id is not None:
                ...
                self._append_node(
                    replace(
                        ...
                        extent=polygon_to_flat(room.polygon),
                        last_seen_ts=ts,
                        metadata=metadata,
                    ),
                    ts,
    ...
    def set_region_geometry(
        ...
        updated = replace(
            ...
            extent=polygon_to_flat(geometry.polygon),
            last_seen_ts=ts,
            metadata={
                **node.metadata,
