...
over the rooms via tool_scene_graph's figure::
    ...
    uv run python dimos/agents/skills/tool_scene_graph_regions.py \
        --db go2_short --out /tmp/region_join_check

Viewer check (manual): start the replay daemon with this seeded DB —

    uv run dimos -o scenegraphskillcontainer.sightings_db=<out>/scene_graph.db \
...
from dimos.agents.skills.scene_graph import SceneGraphSkillContainer, load_pose_trail
from dimos.mapping.occupancy.rooms.polygons import distance_to_polygon, points_in_polygon
...
def main() -> None:
    ...
    with SceneGraph(scene_db) as seed_graph:
        with RoomStore(scene_db) as store:
            store.save(segmentation, source=f"tool_scene_graph_regions:{args.db}")
