...
def trail_db(tmp_path_factory: pytest.TempPathFactory) -> Path:
    db_path = tmp_path_factory.mktemp("scene_graph") / "trail.db"
...
def test_get_scene_reports_time_range_and_honest_room_absence(
    trail_db: Path, tmp_path: Path, make_container: Callable[..., SceneGraphSkillContainer]
) -> None:
...
def test_near(seeded_db: Path, make_container: Callable[..., SceneGraphSkillContainer]) -> None:
    module = make_container(sightings_db=str(seeded_db))
...
def test_nodes_in(seeded_db: Path, make_container: Callable[..., SceneGraphSkillContainer]) -> None:
    ...
    unknown = module.nodes_in("room_99")
...
def test_scan_for_objects_prefers_live_camera_info(
    trail_db: Path,
    tmp_path: Path,
    make_container: Callable[..., SceneGraphSkillContainer],
