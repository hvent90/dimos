...
visit boundaries so a human can re-verify against what the robot saw::
    ...
    uv run python dimos/agents/skills/tool_scene_graph_trail.py \
...
def main() -> None:
    ...
    with SceneGraph(scene_db) as graph:
        with RoomStore(scene_db) as store:
            store.save(segmentation, source=f"tool_scene_graph_trail:{args.db}")
    ...
    container = SceneGraphSkillContainer(trail_db=str(db_path), sightings_db=str(scene_db))
    container.start()
    try:
