...
class SceneAsset:
    ...
    def viewer_kwargs(self) -> dict[str, Any]:
        ...
        if self.browser_collision_path is not None:
            kwargs["browser_collision_path"] = str(self.browser_collision_path)
        return kwargs

    @classmethod
    def from_package(cls, package: ScenePackage) -> SceneAsset:
        """Create a runtime scene asset from a cooked package."""
        if package.visual_path is None:
            raise ValueError("scene package has no browser visual artifact")
        return cls(
            mesh_path=package.visual_path,
...
def get_dimos_office() -> SceneAsset:
    ...
    Reads alignment from the bundled ``dimos_office_mesh.json``. Pull
    happens lazily via ``get_data``.
    """
    ...
    alignment = meta.get("suggested_alignment") or meta["default_alignment_for_dimos"]
