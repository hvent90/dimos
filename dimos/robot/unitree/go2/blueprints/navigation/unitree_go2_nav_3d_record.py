...
"""3d navigation on Go2 with ray tracing, MLS planning, and Point-LIO recording.

Same stack as unitree_go2_nav_3d, plus a PointlioRecorder that captures
PointLio's odometry + lidar into a memory2 SQLite db for later replay/mapping.
"""
...
recording_db_path = "go2_nav_3d_recording.db"
...
unitree_go2_nav_3d_record = autoconnect(
    ...
    # Record PointLio's odometry + lidar; its In ports wire to PointLio's outputs.
    PointlioRecorder.blueprint(db_path=recording_db_path).remappings(
        [
