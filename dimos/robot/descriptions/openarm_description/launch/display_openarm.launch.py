...
def rviz_spawner(context: LaunchContext, bimanual):
    ...
    rviz_config_path = os.path.join(
        get_package_share_directory("openarm_description"), "rviz", rviz_config_file
    )
...
def generate_launch_description():
    arm_type_arg = DeclareLaunchArgument(
        "arm_type", description="Type of arm to visualize (e.g., v10)"
    )
    ...
    bimanual_arg = DeclareLaunchArgument(
        "bimanual", default_value="false", description="Whether to use bimanual configuration"
    )
    ...
    robot_state_publisher_loader = OpaqueFunction(
        function=robot_state_publisher_spawner, args=[arm_type, ee_type, bimanual]
    )

    rviz_loader = OpaqueFunction(function=rviz_spawner, args=[bimanual])
