...
class R1LiteConnection(Module):
    ...
    def start(self) -> None:
        ...
        self._publish_thread = Thread(target=self._publish_loop, name="r1lite-publish", daemon=True)
    ...
    def _setup_control_topics(self) -> None:
        ...
        self._acc_topic = RawROSTopic("/motion_target/chassis_acc_limit", TwistStamped, qos=cmd_qos)
        self._brake_topic = RawROSTopic("/motion_target/brake_mode", Bool, qos=cmd_qos)
    ...
    def _setup_sensor_streams(self) -> None:
        ...
        logger.info("R1Lite sensor streams up: stereo head + 2 wrist RGBD + 2 imus (isolated DDS)")
    ...
    def _on_gripper_command(self, side: str, msg: JointState) -> None:
        ...
        topic = self._cmd_gripper_left_topic if side == "left" else self._cmd_gripper_right_topic
        with self._lock:
    ...
    def _publish_loop(self) -> None:
        ...
        while not self._stop_event.is_set():
            with self._lock:
                ...
                cmd_fresh = (
                    cmd_vel is not None
                    and (time.monotonic() - self._latest_cmd_vel_ts) < self.config.cmd_vel_timeout_s
                )
