...
Hardware wiring lives in `dimos/robot/galaxea/r1pro/` — the connection
Module owns ROS 2, and the coordinator binds its streams via the generic
``transport_lcm`` whole-body and twist adapters. The factories here only
produce :class:`RobotConfig` objects for planning (Drake / MeshCat) and
collision-exclusion metadata.
"""
...
def r1pro_arm(
    ...
    **overrides: Any,
    ...
    """R1 Pro arm RobotConfig for one side (planner / Drake model).

    Both sides reference the same full-body URDF and pick out their 7
    revolute joints. Drake's ``SetAutoRenaming`` deduplicates model names
    when both arms load into the same planning world.

    The hardware-side wiring goes through the ``R1ProConnection`` Module +
    ``transport_lcm`` whole-body adapter, not through this RobotConfig's
    ``adapter_type`` — the default ``"mock"`` is appropriate for planner-only
    workflows.
    """
