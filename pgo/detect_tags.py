...
Self-contained in this repo: vendors apriltags/recording_db under pgo/utils/, the rest resolves
from this clone's own `dimos` package (no dimos3 dependency).

Usage: python pgo/detect_tags.py --rec=PATH [--camera=color_image] [--tag-size=0.10]
...
)  # repo root -> resolves `pgo` and `dimos`
import cv2
...
from pgo.utils.apriltags import (
    _camera_speeds,
...
if not REC_ARG:
    sys.exit(
        "usage: python pgo/detect_tags.py --rec=PATH [--camera=...] [--tag-size=...] "
